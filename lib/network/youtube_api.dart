...
//   bool? getTrending;
//
//   YoutubeAPI(key, {required String type, int maxResults: 10}) {
//     this.type = type;
...
//   }
//
//   Future<List<YT_API>> search(String query, {required String type}) async {
...
//   }
//
//   Future<List<YT_APIPlaylist>> playlist(String channelId, {String? order}) async {
//     this.getTrending = false;
//     Uri url = api!.playlistUri(channelId, order!);
//     var res = await http.get(url, headers: {"Accept": "application/json"});
//     var jsonData = json.decode(res.body);
//     if (jsonData['error'] != null) {
//       throw jsonData['error']['message'];
//     }
//     if (jsonData['pageInfo']['totalResults'] == null) return <YT_APIPlaylist>[];
//     List<YT_APIPlaylist> result = await _getResultPlaylistFromJson(jsonData);
//     return result;
//   }
//
//   /*
...
//     return result;
//   }
//   Future<List<YT_APIPlaylist>> _getResultPlaylistFromJson(jsonData) async {
//     List<YT_APIPlaylist> result = [];
//     if (jsonData == null) return [];
//     //nextPageToken = jsonData['nextPageToken'];
//     //api.setNextPageToken(nextPageToken);
//     int total = jsonData['pageInfo']['totalResults'] <
...
//         ? jsonData['pageInfo']['totalResults']
//         : jsonData['pageInfo']['resultsPerPage'];
//     result = await _getListPlaylistOfYTAPIs(jsonData, total);
//     page = 1;
//     return result;
//   }
//
...
//     for (int i = 0; i < total; i++) {
//       YT_API ytApiObj =
//           YT_API(data['items'][i], getTrendingVideo: getTrending!);
...
//     return result;
//   }
//   Future<List<YT_APIPlaylist>> _getListPlaylistOfYTAPIs(dynamic data, int total) async {
//     List<YT_APIPlaylist> result = [];
//     List<String> playlistIdList = [];
//     for (int i = 0; i < total; i++) {
//       YT_APIPlaylist ytApiObj =
//       new YT_APIPlaylist(data['items'][i], getTrendingVideo: getTrending!);
//       playlistIdList.add(ytApiObj.id!);
//       result.add(ytApiObj);
//     }
//     /*List<YT_VIDEO> videoList = await video(videoIdList);
//     await Future.forEach(videoList, (YT_VIDEO ytVideo) {
...
//     List<YT_API> result = [];
//     Uri url = api!.nextPageUri(getTrending!);
//     var res = await http.get(url, headers: {"Accept": "application/json"});
//     var jsonData = json.decode(res.body);
//
//     if (jsonData['pageInfo']['totalResults'] == null) return <YT_API>[];
...
//     List<YT_API> result = [];
//     Uri url = api!.prevPageUri(getTrending!);
//     var res = await http.get(url, headers: {"Accept": "application/json"});
//     var jsonData = json.decode(res.body);
//
//     if (jsonData['pageInfo']['totalResults'] == null) return <YT_API>[];
